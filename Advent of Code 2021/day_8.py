...
def parse_input(inputs):
    ...
    total_sum = 0
    for s, o in lines:
        ovals = [(v, len(v)) for v in o.split(' ')]
        svals = [(v, len(v)) for v in s.split(' ')]
        vals = {(''.join(sorted(list(v[0]))), v[1]) for v in ovals + svals}
        # lengths = Counter([v[1] for v in vals])
        # unique_vals = [v for v in vals if lengths(v[1]) == 1]
        is_148 = [v for v in vals if v[1] in output_vals]
        remaining = [v for v in vals if v not in is_148]
        pattern_1 = next(v[0] for v in is_148 if v[1] == 2)
        pattern_4 = next(v[0] for v in is_148 if v[1] == 4)
        pattern_7 = next(v[0] for v in is_148 if v[1] == 3)
        pattern_8 = next(v[0] for v in is_148 if v[1] == 7)
        patterns_690 = [v for v in remaining if v[1] == 6]
        patterns_235 = [v for v in remaining if v not in patterns_690]
        signal_a = set(pattern_7).difference(set(pattern_1))
        signals_bd = set(pattern_4).difference(set(pattern_1))
        signals_eg = set(pattern_8).difference(set(pattern_4).union(set(pattern_7)))
        pattern_0 = next(v[0] for v in patterns_690 if len(set(v[0]).difference(pattern_7).difference(signals_eg)) == 1)
        signal_b = set(pattern_0).difference(pattern_7).difference(signals_eg)
        signal_d = signals_bd.difference(signal_b)
        pattern_6 = next(v[0] for v in patterns_690 if len(set(v[0]).difference(signal_a.union(signals_bd).union(signals_eg))) == 1)
        pattern_9 = next(v[0] for v in patterns_690 if v[0] not in [pattern_0, pattern_6])
        signal_c = set(pattern_8).difference(set(pattern_6))
        signal_e = set(pattern_8).difference(set(pattern_9))
        signal_f = set(pattern_1).difference(signal_c)
        signal_g = signals_eg.difference(signal_e)
        pattern_2 = ''.join((signal_a | signal_c | signal_d | signal_e | signal_g))
        pattern_3 = ''.join((signal_a | signal_c | signal_d | signal_f | signal_g))
        pattern_5 = ''.join((signal_a | signal_b | signal_d | signal_f | signal_g))
        digits = {
            "0": pattern_0,
            "1": pattern_1,
            "2": pattern_2,
        ...
        output = []
        for v in ovals:
            digit = next(d for d,p in digits.items() if sorted(list(v[0])) == sorted(list(p)))
            output.append(digit)
        code = ''.join(output)
        logger.info(f"{code=}")
        total_sum += int(code)
...
if __name__ == '__main__':
    ...
    # times.append(timeit.timeit('parse_input(test_input_2)', setup='from __main__ import parse_input, test_input_2', number=1))
    times.append(timeit.timeit('parse_input(puzzle_input)', setup='from __main__ import parse_input, puzzle_input', number=1))
    print(times)