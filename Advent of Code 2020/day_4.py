...
TEST_1_INPUT = """ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
PUZZLE_INPUT = """eyr:2028 iyr:2016 byr:1995 ecl:oth
...
def parse_passports(input_str: str):
    return input_str.split("\n\n")


def parse_fields(passport: str):
    raw_fields = re.split(r"\s", passport)
    fields = {}
...
def has_all_fields(passport: str):
    parsed_fields = parse_fields(passport)
    parsed_keys = parsed_fields.keys()
    all_present = all(
        [expected_field in parsed_keys for expected_field in EXPECTED_FIELDS]
    )
    return all_present


def have_valid_fields(input_str):
    passports = parse_passports(input_str)
    have_valid_fields = [passport for passport in passports if has_all_fields(passport)]
    have_valid_data = [
        passport for passport in have_valid_fields if field_data_is_valid(passport)
    ]
    return len(have_valid_data)
...
def validate_field(key, val):
    ...
    if key == "pid":
        gid_re = r"^\d{9}$"
        return re.search(gid_re, val) is not None
    if key == "cid":
        return True


def is_valid_height(val):
    hgt_re = r"(\d+)(cm|in)$"
    res = re.match(hgt_re, val)
    if not res:
        return False
    measure, units = res.groups()
    measure = int(measure)
    if units == "in":
        return 59 <= measure and 76 >= measure
    if units == "cm":
        return 150 <= measure and 193 >= measure
    return False
...
def is_valid_color(val):
    ...
    if len(val) != 7:
        return False
    color_re = r"#[a-f0-9]{6}"
    return re.search(color_re, val) is not None
...
def field_data_is_valid(passport):
    parsed_fields = parse_fields(passport)
    for field in parsed_fields.items():
...
random_data = [
    {"key": "byr", "val": "2002", "result": True},
    {"key": "byr", "val": "2003", "result": False},
    {"key": "hgt", "val": "60in", "result": True},
    {"key": "hgt", "val": "190cm", "result": True},
    {"key": "hgt", "val": "190in", "result": False},
    {"key": "hgt", "val": "190", "result": False},
    {"key": "hcl", "val": "#123abc", "result": True},
    {"key": "hcl", "val": "#123abz", "result": False},
    {"key": "hcl", "val": "123abc", "result": False},
    {"key": "ecl", "val": "brn", "result": True},
    {"key": "ecl", "val": "wat", "result": False},
    {"key": "pid", "val": "000000001", "result": True},
    {"key": "pid", "val": "0123456789", "result": False},
]

if __name__ == "__main__":
    # print(have_valid_fields(TEST_1_INPUT))
    # print(have_valid_fields(ALL_INVALID))
    # print(have_valid_fields(ALL_VALID))
    # for test in random_data:
    #     result = validate_field(test["key"], test["val"])
    #     print(f"{test}: {result} expected {test['result']}")
    #     if result != test["result"]:
    #         print("TEST FAILED!")
    print(have_valid_fields(PUZZLE_INPUT))