...
def parse_input(input_str):
    ...
    for group in groups:
        ...
        answered_questions = {c for c in cleaned}
        count_any += len(answered_questions)
        individuals = group.splitlines()
        for c in answered_questions:
            contains_c = [ind for ind in individuals if c in ind]
            print(
                f"{c} found in {contains_c}, which is {len(contains_c)} of len{individuals}"
            )
            if len(contains_c) == len(individuals):
                print("All answers contain {c}")
                count_all += 1
    print(count_any, count_all)


if __name__ == "__main__":
    parse_input(TEST_1_INPUT)
