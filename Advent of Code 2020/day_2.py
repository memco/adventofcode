...
def parse_line(line: str):
    ...
    if not line:
        return
    if (parse := re.search(policy_re, line)) :
        policy = parse.group(1)
        password = parse.group(2)
        lengths, char = policy.split(" ")
        lowest, highest = lengths.split("-")
        return (int(lowest), int(highest), char), password


def is_valid(policy, password):
    first, second, char = policy
    # in_pass = [ch for ch in password if ch == char]
    # count = len(in_pass)
    # if count < lowest or count > highest:
    #     return False
    first_is_valid = password[first - 1] == char
    second_is_valid = password[second - 1] == char
    if first_is_valid and second_is_valid:
        return False
    return first_is_valid or second_is_valid


def check_passwords(password_list):
    parsed = [parse_line(line) for line in password_list]
    valid = [is_valid(item[0], item[1]) for item in parsed if item]
    # parsed = map(parse_line, password_list)
    # valid = map(is_valid, parsed)
    count = sum(valid)
    return count


if __name__ == "__main__":
    count = check_passwords(TEST_1_DATA)
    print(count)
    print(count == 2)
    count = check_passwords(DAY_1_DATA)
    print(count)